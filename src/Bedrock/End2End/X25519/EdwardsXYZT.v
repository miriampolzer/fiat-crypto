...
(* Projective coordinates are capitalized to avoid notation clashes. *)
...
(* Converts a projective point p_a to a cached point p_out to prepare it for readdition. 
   Uses the field's parameter d, which is passed as function argument for now. *)
Definition to_cached := func! (p_out, p_a, p_d) {
  fe25519_sub(p_out.half_YmX, p_a.Y, p_a.X);
  fe25519_half(p_out.half_YmX, p_out.half_YmX); (* This doesn't exist yet, work with spec for now. *)
...
(* Let's see if I can get away with one bounds config per point type. *)
(* One problem with these is empty memory assumptions don't make that much sense anymore. *)
...
Local Notation "c '@p5@' p" := (let '(x,y,z,ta,tb) := proj1_sig c in sep (sep (sep (sep (FElem (p .+ 0) x) (FElem (p .+ 40) y)) 
                              (FElem (p .+ 80) z)) (FElem (p .+ 120) ta)) (FElem (p .+ 160) tb)) (at level 10, format "c '@p5@' p").


Definition precomputed_repr (P : precomputed_point) := { p | let '(half_ymx, half_ypx, xyd) := p in
                            P = (feval half_ymx, feval half_ypx, feval xyd) /\
                            bounded_by loose_bounds half_ymx /\ bounded_by loose_bounds half_ypx /\ bounded_by loose_bounds xyd }.
Local Notation "c '@p3@' p" := (let '(half_ymx, half_ypx, xyd) := proj1_sig c in sep (sep (FElem (p .+ 0) half_ymx) (FElem (p .+ 40) half_ypx))
                              (FElem (p .+ 80) xyd)) (at level 10, format "c '@p3@' p").
...
Local Notation "c '@p4@' p" := (let '(half_ymx, half_ypx,z,td) := proj1_sig c in sep (sep (sep (FElem (p .+ 0) half_ymx) (FElem (p .+ 40) half_ypx)) (FElem (p .+ 80) z))
                              (FElem (p .+ 120) td)) (at level 10, format "c '@p4@' p").
...
Global Instance spec_of_add_precomputed : spec_of "add_precomputed" :=
  fnspec! "add_precomputed"
    (p_out p_a p_b : word) /
    (a : point) (b : precomputed_point) (a_repr : projective_repr a) (b_repr : precomputed_repr b) (R : _ -> Prop),
  { requires t m :=
      m =* a_repr @p5@ p_a * b_repr @p3@ p_b * R;  (* TODO memory for p_out *) 
    ensures t' m' :=
      t = t' /\
      (* Oof, I am starting to remember why I disliked using sigma types, you just start pulling your props into your type. 
         Now the fact that p_out is the right output point suddenly depends on the fact that m1add_ is on the curve. *)
      exists p_out : projective_repr (m1add_precomputed_coordinates (coordinates a) b), (* Probably need to tighten bounds for this to work... *)
        m' =* a_repr @p5@ p_a * b @p3@ p_b * R }.
...
Proof.
  ...
  cbv beta match delta [point_repr_bounded_by feval_point_repr coordinates proj1_sig] in *.
  repeat straightline.
  inversion H13.
  repeat single_step.
