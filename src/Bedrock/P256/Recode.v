...
(* This whole file assumes numbers are stored in little endian. It creates a recoding that is also little endian. *)
...
(* Read w bits from p_input starting at i. i is bit-index*)
Definition index_bits :=
  func! (p_input, nbits, i, w) ~> r {
    idx = i >> $3; (* index = i/8 - byte index*)
    a = load1(p_input + idx); (* load the first byte *)
    b = $0;
    if idx + $1 < (nbits + $7) >> $3 { (* check if nbits bits cross a byte boundary. nbits is the number of bits our number has. why is this here? *)
      b = load1(p_input + idx + $1)
    };
    s = a + (b << $8); (* combine the two bytes*)
    t = s >> (i & $7); (* offset = i%8 *)
    r = t & (($1 << w) - $1)
  }.
...
(* Unpack a scalar into a sequence of digits. Specifically p_output will contain nbits/w of length w chunks*)
Definition words_unpack :=
  ...
  (* Convert an array of unisgned w-bit long words into singed w-bit long words. 
  n is the number of words
  ci is carry 
  this returns the last carry, but lukas explained to me that we can prove it will never be set? *)
Definition recode :=
  func! (p_words, ci, n) ~> ci {
      while n {
        x = load1(p_words) + ci;
        unpack! ci = ctime_lt($(2^(w - 1)), x); (* If x is too large to fit into a signed w-bit word, we make it negative and save the carry. *)
...
(* wrapper for recode, ignore carry out and give a 0 for carry in. Requires the input to be small enough to never produce a carry. *)
